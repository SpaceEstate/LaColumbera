...
function admin(c){
  ...
  const panel=async()=>{let d;try{d=await api('adm',{},t)}catch(x){return login()}
    ...
    <div class="admin-card" data-testid="admin-gc"><h2>Trentino Guest Card</h2><p class="book-note">Premi "Mostra tipologie card" per vedere gli ID disponibili sul tuo account (usa già le credenziali Basic Auth di produzione impostate su Vercel): copia l'ID della tipologia giusta e mettilo su Vercel come <b>TGC_CARD_TYPE_ID</b>. Se quella tipologia richiede un attributo, incolla il suo ID qui sotto e premi "Mostra attributi". Il pulsante "Autorizza" serve solo per l'eventuale percorso OAuth alternativo, oggi non necessario.</p><button class="btn btn-line" id="gctip" type="button">Mostra tipologie card</button> <button class="btn btn-line" id="gcauth" type="button">Autorizza (OAuth, riserva)</button><div style="margin-top:8px;display:flex;gap:8px;align-items:center;flex-wrap:wrap"><input id="gcattrid" type="number" placeholder="idTipologiaCard" style="max-width:160px" data-testid="admin-gc-attrid"><button class="btn btn-line" id="gcattr" type="button">Mostra attributi</button></div><pre id="gcout" style="white-space:pre-wrap;word-break:break-word;margin-top:12px;font-size:.85rem"></pre></div>
    ...
    // ---- Guest Card: autorizzazione OAuth (riserva, non necessaria col percorso Basic Auth attivo) ----
    $('#gcauth').onclick=async()=>{const o=$('#gcout');o.textContent='Reindirizzo a Trentino Marketing…';try{const{url}=await api('gc_oauth_start',{},t);location.href=url}catch(x){o.textContent='Errore: '+x.message}};
    ...
    // ---- Guest Card: attributi di una tipologia ----
    $('#gcattr').onclick=async()=>{const o=$('#gcout'),id=$('#gcattrid').value.trim();if(!id){o.textContent='Inserisci prima l\'idTipologiaCard (dal pulsante "Mostra tipologie card")';return}o.textContent='Carico…';try{o.textContent=JSON.stringify(await api('gc_attributi',{idTipologiaCard:id},t),null,2)}catch(x){o.textContent='Errore: '+x.message}};
    // ---- Aggiungi evento ----
