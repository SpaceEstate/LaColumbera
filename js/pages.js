...
function admin(c){
  ...
  const panel=async()=>{let d;try{d=await api('adm',{},t)}catch(x){return login()}
    ...
    <div class="admin-card" data-testid="admin-gc"><h2>Trentino Guest Card</h2><p class="book-note">Prima autorizza l'accesso (una tantum, poi si rinnova da solo). Poi puoi mostrare le tipologie di card del tuo account: copia l'ID della tipologia giusta e mettilo su Vercel come <b>TGC_CARD_TYPE_ID</b>.</p><button class="btn btn-line" id="gcauth" type="button">Autorizza Trentino Guest Card</button> <button class="btn btn-line" id="gctip" type="button">Mostra tipologie card</button><pre id="gcout" style="white-space:pre-wrap;word-break:break-word;margin-top:12px;font-size:.85rem"></pre></div>
    ...
    // ---- Guest Card: autorizzazione OAuth (una tantum) ----
